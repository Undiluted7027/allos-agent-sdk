...
if [ -n "$GROQ_API_KEY" ]; then
    ...
    run_demo allos "'Explain the concept of 'Agentic Workflow' in one sentence.' --provider groq --model llama-3.1-8b-instant --no-tools"
else
...
if [ -n "$OPENAI_API_KEY" ]; then
    ...
    run_demo allos "'Create a file named demo_script.py that prints 'Hello Allos'. Verify it exists.' --provider openai --model gpt-4o --auto-approve"
else
...
if [ -n "$MISTRAL_API_KEY" ]; then
    ...
    run_demo allos "'Delete the file demo_script.py.' --provider mistral --model mistral-small-latest --auto-approve"
fi
