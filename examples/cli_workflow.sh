...
if [ -n "$GROQ_API_KEY" ]; then
    ...
    run_demo allos "Explain the concept of 'Agentic Workflow' in one sentence." \
      --provider groq \
...
if [ -n "$OPENAI_API_KEY" ]; then
    ...
    run_demo allos "Create a file named demo_script.py that prints 'Hello Allos'. Verify it exists." \
      --provider openai \
      --model gpt-4o \
      --auto-approve
else
...
header "4. CLEANUP (Mistral)"
if [ -n "$MISTRAL_API_KEY" ]; then
    echo "Using a different provider to clean up..."
    run_demo allos "Delete the file demo_script.py." \
      --provider mistral \
